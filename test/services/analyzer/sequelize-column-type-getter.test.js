...
describe('services > column type getter', () => {
  ...
  describe('using postgresql', () => {
    ...
    it('should not handle MONEY type', async () => {
      ...
      expect(computedType).toBeNull();
      expect(warnSpy).toHaveBeenCalledWith(`Type ${chalk.bold('MONEY')} is not handled: The column ${chalk.bold('paying')} won't be generated by Forest CLI. If you need it please create it manually.`);
    ...
    describe('using mssql', () => {
      it('should not handle MONEY type', async () => {
        ...
        await sequelizeHelper.dropAndCreate('customers');
        const columnTypeGetter = new ColumnTypeGetter(databaseConnection, 'dbo');
        const computedType = await columnTypeGetter.perform({ type: 'MONEY' }, 'paying', 'customers');

        expect(computedType).toBeNull();
        expect(warnSpy).toHaveBeenCalledWith(`Type ${chalk.bold('MONEY')} is not handled: The column ${chalk.bold('paying')} won't be generated by Forest CLI. If you need it please create it manually.`);

        await sequelizeHelper.drop('customers', 'mssql');
        await sequelizeHelper.close();
      });
    });
  });
