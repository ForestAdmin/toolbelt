...
const { loginValid, loginInvalid, loginInvalidOidc, loginValidOidc } = require('../fixtures/api');
const { testEnvWithoutSecret } = require('../fixtures/env');

describe('login', () => {
  describe('with email in args', () => {
    describe('with bad token in args', () => {
      it('should display invalid token', () =>
        testCli({
          commandClass: LoginCommand,
          commandArgs: ['-e', 'smile@gmail.com', '-t', '__invalid_token__'],
          std: [{ err: '× Invalid token. Please enter your authentication token.' }],
        }));
    });
    describe('with valid token in args', () => {
      it('should login successful', () =>
        testCli({
          ...
          commandClass: LoginCommand,
          commandArgs: ['-e', 'smile@gmail.com', '-t', jwt.sign({}, 'key', { expiresIn: '1day' })],
          std: [{ out: '> Login successful' }],
        }));
    });
    ...
    describe('with an invalid password in args', () => {
      it('should display incorrect password', () =>
        testCli({
          ...
          api: () => loginInvalid(),
          std: [{ err: '× Incorrect email or password.' }],
        }));
    });
  });
