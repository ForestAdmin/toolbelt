...
describe('environments:create', () => {
  ...
  describe('without a logged-in user', () => {
    it('should returns the freshly created environment', () =>
      testCli({
        ...
        additionnalStep: plan =>
          plan.replace('utils/keyGenerator', { generate: () => 'myAuthSecret' }),
        api: [() => loginValidOidc(), () => getProjectListValid(), () => createEnvironmentValid()],
        prompts: [
          {
            in: [
              {
  ...
  describe('when creation fails', () => {
    ...
    describe('with another error having details', () => {
      it('should log error and exit 1', () =>
        testCli({
          env: testEnvWithoutSecret,
          token: 'any',
          commandClass: EnvironmentCreateCommand,
          ...
          api: [() => createEnvironmentDetailedError()],
          std: [{ err: '× dummy test error' }],
          exitCode: 1,
        }));
    });
  });
