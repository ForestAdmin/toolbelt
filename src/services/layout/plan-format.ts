...
/** Map a Forest API error to an actionable message (422 path → JSON key, 403 premium…). */
export function explainApiError(error: LayoutApiError, sentOps: PlannedOp[]): string {
  ...
  if (error.status === 403) {
    ...
    if (premium) {
      return (
        `Premium feature required (pack « ${premium.premiumPack} ») for ${premium.jsonPath}.\n` +
